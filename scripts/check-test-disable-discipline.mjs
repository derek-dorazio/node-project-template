...
const files = walkFiles(['tests', 'packages', 'clients/<projectName>/src'], {
  extensions: ['ts', 'tsx'],
});
...
const skipStoryPattern = /SKIP:\s*bd-#[a-z0-9]+(?:\.[0-9]+)?\b/i;
...
for (const filePath of files) {
  if (skippedFilePattern.test(filePath)) {
    findings.push({
      ...
      message: 'Skipped test files/directories require a SKIP: bd-#* story marker.',
    });
    continue;
  }
  ...
  lines.forEach((line, index) => {
    ...
    findings.push({
      ...
      message: 'Disabled/todo/failing tests require an adjacent SKIP: bd-#* story marker.',
    });
  });
}
