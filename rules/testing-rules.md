...
2. Add a leading comment immediately above the skip with the story ID and one-line reason:
   ```typescript
   // SKIP: bd-#312 — flaky against ephemeral DB; un-skip after migration to test-containers
...
A grep for the markers above (`grep -rE '\.(skip|todo|fails|failing)\(|^x(it|test|describe)\(' tests/ packages/*/src/ clients/*/src/`) should return zero matches that lack an adjacent `SKIP: bd-#NNN` comment. This grep belongs in the `lint` or a dedicated `test:no-undocumented-skips` script.

Riley scans for this on every review:

- Any skipped/todo/expected-fail test introduced by the slice without a `SKIP: bd-#NNN` comment is a **TEST / HIGH** finding and blocks merge.
