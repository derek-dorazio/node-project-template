...
- `rules/architecture-rules.md` (esp. §3 No Mock Data, §3A Provider/Adapter Registry Discipline, §6 App Identity and Access)
- `rules/testing-rules.md` (esp. §2C Forbidden Application-Code Patterns)

## What To Check

### Auth & permission boundaries

- Is every new mutating route guarded by the right authority preHandler?
- Are auth checks placed at the route boundary, not scattered as inline role checks in handlers?
- Are role-elevation paths (admin / privileged / impersonation) properly gated and audit-logged where applicable?
...
- Do API responses include only intended DTO fields? Watch for raw Prisma rows leaking through, missing mapper application, untyped JSON passthroughs.
...
- Do list endpoints respect tenancy / scoping boundaries, or could a member of one scope query another's data?
