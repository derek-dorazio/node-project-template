 ...
 *   OPENAPI_OUTPUT_PATH — override the output path (used by api:check)
 */
...
async function main() {
  ...
  // sub-schemas are reused. These local refs (e.g. "#/properties/foo") are
  // relative to the individual schema node, not the spec root, so
